function varargout = dicm2nii_custom(src, niiFolder, fmt)
...
d=hh{1,2}.SpacingBetweenSlices(1,1);
% pos=hh{1,3}.ImagePositionPatient(2,1)
for k=3:s(2)-1
    if ~isempty(hh{k}) && ~isempty(hh{k-1})
        k
        hh{1,k}.ImagePositionPatient(3,1)= hh{1,(k-1)}.ImagePositionPatient(3,1)+d;
%         hh{1,k}.ImagePositionPatient(3,1)= pos;
%         pos=pos+d;
    end
end
...
switch cmd
    case 'do_convert'
        ...
        drawnow;
        dicm2nii_custom(src, dst, rstFmt);
