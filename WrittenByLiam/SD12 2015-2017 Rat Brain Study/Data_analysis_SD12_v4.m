...
myData.loadNames_Mag = cell(myData.NumofAn, max(myData.PostScans));
myData.loadNames_Real= cell(myData.NumofAn, max(myData.PostScans));
myData.loadNames_Imag= cell(myData.NumofAn, max(myData.PostScans));
myData.loadNames_Map174= cell(myData.NumofAn,1);
myData.loadNames_Map106= cell(myData.NumofAn,1);
myData.loadNames_Map59= cell(myData.NumofAn,1);

    for i=1:1:myData.NumofAn
        an_Num=sprintf('%d', i);
        
        myData.loadNames_Map174(i)=cellstr(strcat('map_rSD12_',an_Num,'_UTE3D_UTEscan',sprintf('%d',(myData.PostScans(i)+1)),'_MAGNITUDE_IMAGE_174_REGION.nii'));
        myData.loadNames_Map106(i)=cellstr(strcat('map_rSD12_',an_Num,'_UTE3D_UTEscan',sprintf('%d',(myData.PostScans(i)+1)),'_MAGNITUDE_IMAGE_106_REGION.nii'));
        myData.loadNames_Map59(i)=cellstr(strcat('map_rSD12_',an_Num,'_UTE3D_UTEscan',sprintf('%d',(myData.PostScans(i)+1)),'_MAGNITUDE_IMAGE_59_REGION.nii'));
        
            for j=1:1:(myData.PostScans(i)+1)
                UTE_Num=sprintf('%d', j);
                 myData.loadNames_Mag(i,j)=cellstr(strcat('rSD12_',an_Num,'_UTE3D_UTEscan',UTE_Num,'_MAGNITUDE_IMAGE.nii'));
                 myData.loadNames_Real(i,j)=cellstr(strcat('rSD12_',an_Num,'_UTE3D_UTEscan',UTE_Num,'_REAL_IMAGE.nii'));
                 myData.loadNames_Imag(i,j)=cellstr(strcat('rSD12_',an_Num,'_UTE3D_UTEscan',UTE_Num,'_IMAGINARY_IMAGE.nii'));
            end
...
% and each of the Atlas_SIZE regions capilary density and cerebral blood volume
% values. 
Atlas_SIZE=59; % Can be : 174, 106, 59
...
for k=1:1:myData.NumofAn
	...
	Ib = csvread(csvFile);
        ...
        if Atlas_SIZE==174
        AlignedAtlas = load_nii(char(myData.loadNames_Map174(k)));
        elseif Atlas_SIZE==106
        AlignedAtlas = load_nii(char(myData.loadNames_Map106(k)));    
        elseif Atlas_SIZE==59
        AlignedAtlas = load_nii(char(myData.loadNames_Map59(k)));
        end
        
        PreCon_R = load_nii(char(myData.loadNames_Real(k,1)));
        PreCon_I = load_nii(char(myData.loadNames_Imag(k,1)));
        
        for j=1:1:myData.PostScans(k)
            clear loadname PostCon_R PostCon_I Mask PreConImg PostConImg 
            clear I1_peak I1_avg I2_peak I2_avg I_precon I_postcon

            PostCon_R = load_nii(char(myData.loadNames_Real(k,j+1)));
            PostCon_I = load_nii(char(myData.loadNames_Imag(k,j+1)));
                 
                % Scan through ATLAS regions per animal
                for i = 1:1:Atlas_SIZE
                ...
                PreConImg_R = double(PreCon_R.img);
                PreConImg_I = double(PreCon_I.img);
                PostConImg_R= double(PostCon_R.img);
                PostConImg_I= double(PostCon_I.img);
    
                [I1_peak,I1_avg,I1_std] = get_Ipeak_Iavg_complex(PreConImg_R, PreConImg_I, Mask);
                [I2_peak,I2_avg,I2_std] = get_Ipeak_Iavg_complex(PostConImg_R,PostConImg_I ,Mask);
...
M1=zeros(Atlas_SIZE,12);M2=zeros(Atlas_SIZE,12);M3=zeros(Atlas_SIZE,12);M4=zeros(Atlas_SIZE,12);
...
for i=1:1:12
    ...
    M_STAT=zeros(Atlas_SIZE,20);

	for j=1:1:Atlas_SIZE
