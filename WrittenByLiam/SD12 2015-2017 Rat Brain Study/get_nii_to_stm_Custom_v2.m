function [] = get_nii_to_stm_Custom_v2(CurrentNii, saveName, Percent, Reverse, Scale)
...
CurrentImg=CurrentImg*f;

if Reverse==1
    CurrentImg=f-CurrentImg;
end

if Scale==1
    CurrentData=nonzeros(CurrentImg);
    ScaleFactor=f/max(CurrentData);
    CurrentImg=CurrentImg*ScaleFactor;
end

CurrentData=nonzeros(CurrentImg);
s=size(CurrentImg);
CurrentImg(s(1),s(2),s(3))=2*max(CurrentData);

CurrentNii.img=int32(CurrentImg);
