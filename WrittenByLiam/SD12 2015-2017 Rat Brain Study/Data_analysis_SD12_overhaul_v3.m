...
[~, BloodFromPeakStd_r, ~, ~] =get_BloodValues(1);
[~, BloodFromPeakStd_i, ~, ~] =get_BloodValues(2);
[BloodFromPeak_m, BloodFromPeakStd_m, ~, ~] =get_BloodValues(3);

BloodFromPeakStd=(BloodFromPeakStd_r+BloodFromPeakStd_i)./2;
BloodFromPeak=(BloodFromPeak_m.^2 - BloodFromPeakStd.^2).^(1/2);
...
for t=1:1:3
    ...
    if exist(filename, 'file')
        ...
        [I_avg_pre_r, I_std_pre_r, I_avg_post_r, I_std_post_r, ~, ~, ~, ~] =get_IntensityPerRegion(TotRegions, 1, peakget);
        [I_avg_pre_i, I_std_pre_i, I_avg_post_i, I_std_post_i, ~, ~, ~, ~] =get_IntensityPerRegion(TotRegions, 2, peakget);
        [I_avg_pre_m, I_std_pre_m, I_avg_post_m, I_std_post_m, ~, ~, ~, ~] =get_IntensityPerRegion(TotRegions, 3, peakget);
        
        I_std_pre=(I_std_pre_r+I_std_pre_i)./2;  
        I_avg_pre=(I_avg_pre_m.^2 - I_std_pre.^2).^(1/2);
        
        I_std_post=(I_std_post_r+I_std_post_i)./2 ;
        I_avg_post=(I_avg_post_m.^2 - I_std_post.^2).^(1/2);
