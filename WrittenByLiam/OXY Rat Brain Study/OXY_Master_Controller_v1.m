...
for i=1:s(1)
    for j=1:2
        ...
        for k=1:NumScans
            ...
            filelist(1,1)={strcat(Path, '\r', Subject, 'scan', scn_num,'_MAGNITUDE_IMAGE_x100.nii,1')};
        end
        ...
        for k=1:NumScans
            ...
            save_nii(NewNii,file);
            ResliceSPMcontroller(file, dim, dim, dim)
            filelist(k+1,1)={strcat(Path, '\r', Subject, 'scan', scn_num,'_MAGNITUDE_IMAGE.nii,1')};
        end
        ...
        for k=1:NumScans
            scn_num=sprintf('%d', k);
            Subject=strcat(model, '_', State, '_R',an_num, '_', modality);
            filelist(k,1)={strcat(Path, '\', Subject, 'scan', scn_num,'_MAGNITUDE_IMAGE.nii,1')};   
        end
...
%             {
            ...
            filelist(1,1)
            filelist(2,1)
            %'E:\Backup And Sync\unshared research\PostAddiction\EXTRACTED_RENAMED_WORKING\OXY_QUTECE_POST_R1_UTE3Dscan3_MAGNITUDE_IMAGE.nii,1'
            %'E:\Backup And Sync\unshared research\PostAddiction\EXTRACTED_RENAMED_WORKING\OXY_QUTECE_POST_R1_UTE3Dscan4_MAGNITUDE_IMAGE.nii,1'
            %'E:\Backup And Sync\unshared research\PostAddiction\EXTRACTED_RENAMED_WORKING\OXY_QUTECE_POST_R1_UTE3Dscan1_MAGNITUDE_IMAGE.nii,1'
            %'E:\Backup And Sync\unshared research\PostAddiction\EXTRACTED_RENAMED_WORKING\OXY_QUTECE_POST_R1_UTE3Dscan2_MAGNITUDE_IMAGE.nii,1'
%             }
            }';
        matlabbatch{1}.spm.spatial.realign.estwrite.eoptions.quality = 0.99;
        matlabbatch{1}.spm.spatial.realign.estwrite.eoptions.sep = 0.2;
...
%% Regional Difference Extraction
...
folderPre='PreAddiction';
folderPost='PostAddiction';
subfolder1='EXTRACTED_RENAMED_RESLICED_UTE_WORKING';
subfolder2='MAP_FIXED';
...
clearvars -except myData NumReg
...
for i=1:NumAns
    for k=1:NumReg+1
        ...
        if ~isempty(PreOxyAve(i,k)) && ~isempty(PostOxyAve(i,k))
